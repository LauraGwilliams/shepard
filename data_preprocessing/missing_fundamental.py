...
for i in range(len(conditions)):
    ...
    temp_evoked.save(meg_dir + 'R1201/evokeds/1201_shepard_%s-evoked-ave.fif'%(conditions[i]))
    temp_stc = apply_inverse(temp_evoked, inverse_operator, lambda2, method = 'dSPM')
    temp_stc.save(meg_dir + 'stc/%s/1201_shepard_%s'%(conditions[i],conditions[i]))
