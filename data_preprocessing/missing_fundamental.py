...
Eb_tones = ['Eb_312', 'Eb_349', 'Eb_392', 'Eb_415', 'Eb_466', 'Eb_524', 'Eb_587', 'Eb_624']

all_tones = [A_tones, C_tones, Eb_tones]

A_freqs = ['220', '247', '277', '294', '330', '370', '415', '440']
C_freqs = ['262', '294', '330', '349', '392', '440', '494', '524']
Eb_freqs = ['312', '349', '392', '415', '466', '524', '587', '624']

all_freqs = [A_freqs, C_freqs, Eb_freqs]

freqs = ['220', '233', '247', '262', '277', '294', '312', '330', '349', '370',
'392', '415', '440', '466', '494', '524', '587', '624']
...
for i in range(len(conditions)):
    ...
    temp_evoked.save(meg_dir + subj + '/evokeds/' + subj + '_shepard_%s-evoked-ave.fif'%(conditions[i]))
...
for j in range(len(freqs)):
    ...
    temp_evoked.save(meg_dir + subj + '/evokeds/' + subj + '_shepard_%s-evoked-ave.fif'%(freqs[j]))
    #temp_stc = apply_inverse(temp_evoked, inverse_operator, lambda2, method = 'dSPM')
...
for k in range(len(freqs)):
    ...
    temp_evoked.save(meg_dir + subj + '/evokeds/' + subj + '_shepard_%spure-evoked-ave.fif'%(freqs[k]))
    #temp_stc = apply_inverse(temp_evoked, inverse_operator, lambda2, method = 'dSPM')
...
for l in range(len(freqs)):
    ...
    temp_evoked.save(meg_dir + subj + '/evokeds/' + subj + '_shepard_%spar-evoked-ave.fif'%(freqs[l]))
    #temp_stc = apply_inverse(temp_evoked, inverse_operator, lambda2, method = 'dSPM')
...
for m in range(len(keys)):
    for n in range(8):
        ...
        temp_evoked.save(meg_dir + subj + '/evokeds/' + subj + '_shepard_%s-evoked-ave.fif'%(all_tones[m][n]))
        #temp_stc = apply_inverse(temp_evoked, inverse_operator, lambda2, method = 'dSPM')
